...
# Execute method
  def execute(run_method)
    ...
    when "tcp_quick_vo_scripts"     then cmd_infix = "-Pn -sSV -O -T4 --min-hostgroup 128 --script 'auth,default,discovery,version,vuln'"
    when "tcp_quick"                then cmd_infix = "-Pn -sS -T4 --min-hostgroup 128"
    when "tcp_quick_vo"             then cmd_infix = "-Pn -sSV -O -T4 --min-hostgroup 128"
    when "tcp_udp_full"             then cmd_infix = "-Pn -sSU -p0-65535 -T4 --min-hostgroup 128"
    when "tcp_udp_quick_vo_scripts" then cmd_infix = "-Pn -sSUV -O -T4 --min-hostgroup 128 --script 'auth,default,discovery,version,vuln'"
    when "tcp_udp_quick"            then cmd_infix = "-Pn -sSU -T4 --min-hostgroup 128"
    when "tcp_udp_quick_vo"         then cmd_infix = "-Pn -sSUV -O -T4 --min-hostgroup 128"
    ...
    when "udp_quick_vo_scripts"     then cmd_infix = "-Pn -sUV -O -T4 --min-hostgroup 128 --script 'auth,default,discovery,version,vuln'"
    when "udp_quick"                then cmd_infix = "-Pn -sU -T4 --min-hostgroup 128"
    when "udp_quick_vo"             then cmd_infix = "-Pn -sUV -O -T4 --min-hostgroup 128"
    end
